# fix-d48c085b1867a57d — Fix: Failing dialog spec: bot responds to a plain-text update
...
The Tests-gate replay harness reported spec "bot responds to a plain-text update" as failing. 3 spec(s) failed: [bot responds to a plain-text update plain text still works photo message receives help hint fallback]

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-d48c085b1867a57d.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-d48c085b1867a57d.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
