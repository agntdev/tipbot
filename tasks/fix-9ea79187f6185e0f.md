# fix-9ea79187f6185e0f — E3T1: Token env var name differs from task spec

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TIP

The E3T1 task spec states: "Check that the TELEGRAM_BOT_TOKEN environment variable is present and valid." The implementation in `src/index.ts` reads `process.env.BOT_TOKEN` — omitting the `TELEGRAM_` prefix. The validation logic itself is correct (presence check, regex format validation, clear error messages, proper exit codes), but the env var name does not match the specification.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-9ea79187f6185e0f.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-9ea79187f6185e0f.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
