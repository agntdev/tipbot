# fix-0dc76666db6eda34 — Fix: Failing dialog spec: bot still responds to plain text
...
The Tests-gate replay harness reported spec "bot still responds to plain text" as failing. 3 spec(s) failed: [/count returns total tips served /count reflects a tip that was served bot still responds to plain text]

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-0dc76666db6eda34.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-0dc76666db6eda34.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
