# fix-3467a4d863558ba7 — SQLite incr() not wrapped in a transaction — violates spec persistence requirements

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TIP

`spec.md` line 40-41 requires: *"Use transactions and WAL mode so concurrent handlers can increment safely."* The `SqlitePersistentStore.incr()` method (`src/persistent.ts:102-113`) executes three separate SQL statements (`INSERT OR IGNORE`, `UPDATE … +1`, `SELECT`) without wrapping them in `BEGIN`/`COMMIT`. While sql.js is single-threaded (WebAssembly), the spec explicitly requires transactions for correctness guarantees, and the code doesn't comply.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-3467a4d863558ba7.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-3467a4d863558ba7.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
