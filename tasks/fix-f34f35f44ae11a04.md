# fix-f34f35f44ae11a04 — .env.example uses wrong env var name — BOT_TOKEN instead of TELEGRAM_BOT_TOKEN

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TIP

`.env.example` contains `BOT_TOKEN=` but the runtime entry (`src/index.ts:7`) reads `TELEGRAM_BOT_TOKEN`. A developer following the example would set the wrong variable, get a runtime exit (`TELEGRAM_BOT_TOKEN is required`), and be confused. The Dockerfile comment also references `BOT_TOKEN`.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-f34f35f44ae11a04.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-f34f35f44ae11a04.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
