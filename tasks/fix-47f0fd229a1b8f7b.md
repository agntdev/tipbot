# fix-47f0fd229a1b8f7b — Fix: Failing dialog spec: photo message receives help hint fallback
...
The Tests-gate replay harness reported spec "photo message receives help hint fallback" as failing. 3 spec(s) failed: [bot responds to a plain-text update plain text still works photo message receives help hint fallback]

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-47f0fd229a1b8f7b.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-47f0fd229a1b8f7b.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
