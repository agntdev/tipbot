# fix-0145c42ac5ee1fb3 — Fix: Failing dialog spec: plain text still works
...
The Tests-gate replay harness reported spec "plain text still works" as failing. 3 spec(s) failed: [bot responds to a plain-text update plain text still works photo message receives help hint fallback]

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-0145c42ac5ee1fb3.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-0145c42ac5ee1fb3.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
